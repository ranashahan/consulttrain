...
/*!50001 CREATE VIEW `vsession` AS SELECT 
 ...
 1 AS `licensetypeid`,
 1 AS `licenseexpiry`,
 1 AS `licenseverified`,
 1 AS `drivercontractorid`,
 1 AS `permitnumber`,
 1 AS `permitissue`,
 1 AS `permitexpiry`,
 1 AS `drivercode`,
...
/*!50001 VIEW `vsession` AS select `s`.`id` AS `id`,`s`.`name` AS `name`,`s`.`sessiondate` AS `sessiondate`,`s`.`locationid` AS `locationid`,`s`.`resultid` AS `resultid`,`s`.`stageid` AS `stageid`,`s`.`titleid` AS `titleid`,`s`.`vehicleid` AS `vehicleid`,`s`.`totalscore` AS `totalscore`,`s`.`classdate` AS `classdate`,`s`.`yarddate` AS `yarddate`,`s`.`weather` AS `weather`,`s`.`traffic` AS `traffic`,`s`.`route` AS `route`,`s`.`quizscore` AS `quizscore`,`s`.`comment` AS `comment`,`s`.`formid` AS `formid`,`s`.`active` AS `active`,`d`.`id` AS `driverid`,`d`.`name` AS `drivername`,`d`.`nic` AS `nic`,`d`.`dob` AS `dob`,`d`.`designation` AS `designation`,`d`.`department` AS `department`,`d`.`bloodgroupid` AS `bloodgroupid`,`d`.`visualid` AS `visualid`,`d`.`licensenumber` AS `licensenumber`,`d`.`licensetypeid` AS `licensetypeid`,`d`.`licenseexpiry` AS `licenseexpiry`,`d`.`licenseverified` AS `licenseverified`,`d`.`contractorid` AS `drivercontractorid`,`d`.`permitnumber` AS `permitnumber`,`d`.`permitissue` AS `permitissue`,`d`.`permitexpiry` AS `permitexpiry`,`d`.`code` AS `drivercode`,`t`.`id` AS `trainerid`,`t`.`name` AS `trainername`,`c`.`id` AS `contractorid`,`c`.`name` AS `contractorname`,`cc`.`id` AS `clientid`,`cc`.`name` AS `clientname`,json_arrayagg(json_object('id',`a`.`id`,'slavecategoryid',`a`.`slavecategoryid`,'slavecategoryname',`sc`.`name`,'activityid',`a`.`activityid`,'activityname',`aa`.`name`,'assessment_type',`a`.`assessment_type`,'score',`a`.`score`,'assessmentdate',`a`.`assessmentdate`)) AS `assessments` from ((((((((((`session` `s` left join `session_driver` `sd` on((`s`.`id` = `sd`.`session_id`))) left join `driver` `d` on((`sd`.`driver_id` = `d`.`id`))) left join `session_trainer` `st` on((`s`.`id` = `st`.`session_id`))) left join `trainer` `t` on((`st`.`trainer_id` = `t`.`id`))) left join `session_contractor` `ct` on((`s`.`id` = `ct`.`session_id`))) left join `contractor` `c` on((`ct`.`contractor_id` = `c`.`id`))) left join `client` `cc` on((`c`.`clientid` = `cc`.`id`))) left join `assessment` `a` on((`s`.`id` = `a`.`sessionid`))) left join `slavecategory` `sc` on((`a`.`slavecategoryid` = `sc`.`id`))) left join `activity` `aa` on((`a`.`activityid` = `aa`.`id`))) group by `s`.`id`,`s`.`name`,`s`.`sessiondate`,`s`.`locationid`,`s`.`totalscore`,`d`.`id`,`d`.`name`,`d`.`nic`,`d`.`permitnumber`,`t`.`id`,`t`.`name`,`c`.`id`,`c`.`name`,`cc`.`id`,`cc`.`name` */;
...
--
/*!50003 DROP PROCEDURE IF EXISTS `categories` */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `categories`()
BEGIN
SELECT v.id, v.name, v.description, p.username as createdby, v.created_at FROM category v 
...
c.website, c.agentname, c.agentnumber, c.industriesid, p.username as createdby, c.created_at from client c 
...
BEGIN
SELECT v.id, v.name, v.description, p.username as createdby, v.created_at FROM course v 
join users p on p.userid = v.createdby
order by v.name asc;
END ;;
DELIMITER ;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `getAllExpireDriver` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `getAllExpireDriver`(IN p_expire varchar(50))
BEGIN
SET @query = CONCAT('select d.id as Database_ID, 
d.name as Driver_Name, 
c.name as Driver_Contractor,
cc.name as Driver_Client, 
gender as Driver_Gender,
dob as Driver_DateOfBirth,
nic as Driver_NationIdentityCard_No,
nicexpiry as Driver_NationIdentityCard_Expiry,
licensenumber as Driver_License_No,
l.name as Driver_License_Type,
licenseexpiry as Driver_License_Expiry,
designation as Driver_Designation,
department as Driver_Department,
permitnumber as Driver_Permit_No,
permitissue as Driver_Permit_Issue,
permitexpiry as Driver_Permit_Expiry, 
medicalexpiry as Driver_Medical_Expiry, 
b.name as Driver_BloodGroup,
v.name as Driver_Visual
from driver d 
left join contractor c on d.contractorid = c.id
left join licensetype l on d.licensetypeid = l.id 
left join bloodgroup b on d.bloodgroupid = b.id
left join visual v on d.visualid = v.id
left join client cc on c.clientid = cc.id
where d.', p_expire, ' < NOW() AND d.active = 1 order by d.created_at desc;');
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `getAllIndustries` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `getAllIndustries`()
BEGIN
SELECT i.id, i.name, i.description, p.username as createdby, i.created_at FROM industries i 
join users p on p.userid = i.createdby
order by i.name asc;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `getAllTrainings` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `getAllTrainings`()
BEGIN
select t.id,
t.name,
t.courseid,
...
t.active,
p.username as createdby, t.created_at FROM training t 
join users p on p.userid = t.createdby
WHERE t.active = 1
...
FROM driver d
LEFT JOIN users p ON p.userid = d.createdby
LEFT JOIN users u ON u.userid = d.modifiedby
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `getTrainingByID` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `getTrainingByID`(idTraining int)
BEGIN
select t.id,
t.name,
t.courseid,
...
LEFT JOIN users u ON u.userid = t.modifiedby
WHERE t.active = 1 and t.id=idTraining;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_driver`(
    ...
    IN d_Gender VARCHAR(15),
    IN d_DOB DATE,
   IN d_NIC NVARCHAR(255),
   IN d_NICExpiry DATE,
   IN d_LicenseNumber VARCHAR(255),
   IN d_LicenseTypeId INT,
   IN d_LicenseExpiry DATE,
   IN d_PermitNumber VARCHAR(255),
   IN d_PermitIssue DATE,
   IN d_PermitExpiry DATE,
   IN d_LicenseVerified INT,
  IN  d_Designation VARCHAR(255),
  IN  d_Department VARCHAR(255),
  IN d_MedicalExpiry DATE,
  IN d_BloodGroupId INT,
  IN  d_ContractorId INT,
  IN  d_VisualId INT,
  IN  d_DDCCount INT,
  IN  d_Experience INT,
  IN  d_Code VARCHAR(255),
  IN  d_Comment VARCHAR(500),
  IN  d_CreatedBy INT)
BEGIN
/*
	DECLARE PermitNumber VARCHAR(255);
    DECLARE PermitIssue DATE;
    DECLARE PermitExpiry DATE;
    DECLARE dcount INT;
    SET PermitIssue= CURDATE();
    SET PermitExpiry = DATE_ADD(PermitIssue, INTERVAL 2 YEAR);
    SET dcount = (select count(id) from driver);
    SET PermitNumber = CONCAT('CNTD-', LPAD(dcount + 1, 6, '0'));
  */  
	
    INSERT INTO Driver (Name, Gender, DOB, NIC, NICExpiry, LicenseNumber, LicenseTypeId, LicenseExpiry, LicenseVerified, Designation, Department, PermitNumber, PermitIssue, PermitExpiry, Medicalexpiry, BloodGroupId, ContractorId, VisualId, DDCCount, Experience, Code, Comment, CreatedBy, ModifiedBy)
    VALUES (d_Name, d_Gender, d_DOB, d_NIC, d_NICExpiry, d_LicenseNumber, d_LicenseTypeId, d_LicenseExpiry, d_LicenseVerified, d_Designation, d_Department, d_PermitNumber, d_PermitIssue, d_PermitExpiry, d_MedicalExpiry, d_BloodGroupId, d_ContractorId, d_VisualId, d_DDCCount, d_Experience, d_Code, d_Comment, d_CreatedBy, d_CreatedBy);
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Error handling: check if required parameters are NULL
	IF p_formid IS NULL OR p_formid=0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Error: form ID is required.';
    END IF;
    ...
    INSERT INTO session (name, sessiondate, locationid,resultid,stageid,titleid,vehicleid,totalscore,classdate,yarddate,weather,traffic,route,quizscore,comment,formid,createdby,modifiedby)
    VALUES (p_sessionname, p_sessiondate, p_locationid,p_resultid,p_stageid,p_titleid,p_vehicleid,p_totalscore,p_classdate,p_yarddate,p_weather,p_traffic,p_route,p_quizscore,p_comment,p_formid,p_userid,p_userid);
...
-- Dump completed on 2025-02-20  2:28:25
